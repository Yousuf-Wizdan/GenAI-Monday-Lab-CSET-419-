...
Original file is located at
    https://colab.research.google.com/drive/1BWtYokCuXC4SHkoyu1ERlNfHDuqgsBdV
...
for epoch in range(1, epochs+1):
    ...
    print(f"Epoch {epoch}/{epochs} | D_loss {d_loss/len(dataloader):.3f} | G_loss {g_loss/len(dataloader):.3f} | D_acc {(100*correct/total):.2f}%")

    if epoch % save_interval == 0:
        with torch.no_grad():
            z = torch.randn(25,noise_dim,device=device)
            samples = G(z)
            save_image(samples, f"generated_samples/epoch_{epoch}.png", nrow=5, normalize=True)
...
for i,img in enumerate(final):
    save_image(img, f"final_generated_images/img_{i:03d}.png", normalize=True)
